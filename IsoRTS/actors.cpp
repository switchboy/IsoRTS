...
void actors::gatherResource()
{
    if (isReallyNextToResource(this->actorCords.x, this->actorCords.y, this->actionPreformedOnTile.x, this->actionPreformedOnTile.y)) {
        ...
        else
        {
            //resource not here!
            std::stringstream error;
            error << "Resources no longer here! Going back to base. Error generated by actor: " << this->actorId;
            gameText.addNewMessage(error.str(), 1);
...
void actors::unloadAndReturnToGathering()
{
    ...
    if (this->actionPreformedOnTile.x >= 0 && this->actionPreformedOnTile.x < MAP_WIDTH && this->actionPreformedOnTile.y >= 0 && this->actionPreformedOnTile.y < MAP_HEIGHT) {
        ...
        else
        {
            //resource not here!
            std::stringstream error;
            error << "Resources no longer here! Going to find alternative! - generated by actor: " << this->actorId;
        ...
        //resource not here!
        std::stringstream error;
        error << "Resource location out of map bounds! - generated by actor: " << this->actorId;
        gameText.addNewMessage(error.str(), 1);
